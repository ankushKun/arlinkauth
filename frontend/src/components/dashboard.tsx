...
import type { WauthUser, SignDataItemResult, SignatureResult, DispatchResult, GitHubLoginOptions } from "arlinkauth";
...
// GitHub scope options for the login screen
const GITHUB_SCOPES = [
  { id: "repo", label: "repo" },
  { id: "read:org", label: "org" },
  { id: "gist", label: "gist" },
...
function ScopeBadge({ 
  ...
  scope: typeof GITHUB_SCOPES[number]; 
  selected: boolean; 
  onToggle: () => void;
}) {
  return (
...
function LoginScreen({
  ...
  loginWithGithub: (options?: GitHubLoginOptions) => void;
  ...
  const handleGithubLogin = () => {
    ...
    loginWithGithub(scopes.length > 0 ? { scopes } : undefined);
  };
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">GitHub scopes (optional)</p>
            <div className="flex flex-wrap gap-2">
              {GITHUB_SCOPES.map(scope => (
                <ScopeBadge
                  key={scope.id}
                  scope={scope}
                  selected={selectedScopes.has(scope.id)}
          ...
          <div className="space-y-3 pt-2">
            <Button onClick={handleGithubLogin} className="w-full" size="lg" variant="outline">
