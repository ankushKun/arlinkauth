...
app.get("/auth/github/callback", async (c) => {
  ...
  const storedState = cookies.split(";").map((s) => s.trim()).find((s) => s.startsWith("oauth_state="))?.split("=")[1];
  ...
  await c.env.DB.prepare(
    `INSERT INTO users (id, github_id, github_username, github_access_token, updated_at)
     VALUES (?1, ?2, ?3, ?4, datetime('now'))
     ON CONFLICT (github_id) DO UPDATE SET
       github_username = excluded.github_username,
       github_access_token = excluded.github_access_token,
       updated_at = datetime('now')`
  ).bind(userId, ghUser.id, ghUser.login, tokenData.access_token).run();
  const row = await c.env.DB.prepare(
    `SELECT id FROM users WHERE github_id = ?1`
  ).bind(ghUser.id).first();
  const actualUserId = row.id;
  const jwt2 = await sign2(
    { sub: actualUserId, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 * 7 },
    c.env.JWT_SECRET,
    "HS256"
  );
  c.header(
  ...
  return c.redirect(`${c.env.FRONTEND_URL}?token=${jwt2}`);
});
...
// .wrangler/tmp/bundle-LIB3Vr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LIB3Vr/middleware-loader.entry.ts
